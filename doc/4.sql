...
create table wb_info (
  ...
  `user_id` int,
  `content` text,
  CONSTRAINT FK_Name FOREIGN key (`user_id`) REFERENCES `wb_user`(id) on delete cascade on update cascade
);
...
select
  ...
  sum(like_num) as t
from wb_user as A
left join wb_info B on A.id = B.user_id
group by
  A.nickname
order by
  t desc
...
--  group by 语句报错需要所有字段必须聚合时添加至配置文件
  --  sql_mode = STRICT_TRANS_TABLES, NO_ZERO_IN_DATE, NO_ZERO_DATE, ERROR_FOR_DIVISION_BY_ZERO, NO_AUTO_CREATE_USER, NO_ENGINE_SUBSTITUTION -- 当数据量较大时，请合理地建立索引来优化你在第 2、3 个问题中给出的 SQL 语句查询效率。
